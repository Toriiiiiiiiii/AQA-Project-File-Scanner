...
class FileScan:
    ...
    def __performScan(self) -> None:
        try:
            with open(self.filePath, "rb") as scanTargetFile:
                self.__fileContents = scanTargetFile.read()
        except:
            print("[ERROR] Could not read file.")
            return
        
        patternQueryResult = databaseCurs.execute("SELECT * FROM patternFile").fetchall()
